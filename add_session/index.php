...
if(isset($_GET['exit'])){
    ...
    if($link == FALSE){
        ...
        if(isset($_POST["add_session"])){
            $mname=$_POST['mname'];
            $hname=$_POST['hname'];
            $sql1 = "SELECT movie_id as mid FROM movies WHERE name='".$mname."'";
            $sql2 = "SELECT hall_id as hid FROM halls WHERE name='".$hname."'";
            $mid = mysqli_fetch_array(mysqli_query($link, $sql1));
            $hid = mysqli_fetch_array(mysqli_query($link, $sql2));
            $sdate=$_POST['sdate'];
            $request4 = "INSERT INTO sessions(movie_id, session_datetime, hall_id) VALUES(".$mid['mid'].", '$sdate', ".$hid['hid'].")";
        ...
        /*      else if(!empty($_GET['mname'])) {
              ...
              $request = "SELECT `name` as mname FROM movies";
